...
static const string tamanho(const BigInt& BI)
{
  string txt=" (";
  ...
  txt += ')';
  return txt; // copy elision; automaticamente move o conteudo de "txt"
}
...
int main()
{
  ...
  do
  {
    ...
    if (opcao == Opcoes_t::DIVIDIR_AMBOS)
    {
      cout << setw(7) << opcao_txt << " =====\n";
      cout << "Quoci:  ";
    }
    else
    {
    ...
    cout << Result << tamanho(Result) << endl;
    ...
    if (opcao == Opcoes_t::OPCAO_INVALIDA)
    {
      if (isdigit(opcao_txt[0]) ||
          ( (opcao_txt[0]=='+' || opcao_txt[0]=='-') &&
            opcao_txt.size()>1 && isdigit(opcao_txt[1]) ))
      {
